...
export default {
  install(Vue, opt) {
    ...
    Vue.prototype.tableHeaderColor = function ({row, column, rowIndex, columnIndex}) {
      if (rowIndex === 0) {
        // return 'background-color:#800080 ;color: #fff;font-weight: 500;'
      ...
      getChainInfoStruct().then(response => {
        ...
        if (response.data.childrens.length == 0 || response.data.childrens == null) {
          ...
          response.data.childrens.forEach((item, index, self) => {
            // chain_list.zh_chain_arr.push({'label': '子链' + (index + 1), 'value': item.chainId,'is_have':true})
            // chain_list.en_chain_arr.push({'label': 'Subchain' + (index + 1), 'value': item.chainId,'is_have':true})
