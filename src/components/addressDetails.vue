      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span :class="is_click(scope.row.txType,scope.row.from)"
                @click="change_address(scope.row.chainId,scope.row.from,scope.row.txType)">{{slice_hash(scope.row.from)}}</span>
        </template>
      </el-table-column>
      <el-table-column
        ...
        <template slot-scope="scope">
          <span :class="is_click_1(scope.row.txType,scope.row.to)"
                @click="change_address_1(scope.row.chainId,scope.row.to,scope.row.txType)">{{slice_hash(scope.row.to)}}</span>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // table_style(e){
      // // :class="(index+1)%4==0?'':'dd_bor',(index+1)%4==1?'':'bo_left'"
      //   let a=''
      //   if((e+1)%4==0){
      //     a=''
      //   }
