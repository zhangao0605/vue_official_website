...
{{scientificCounting(scope.row.txCost)}} TKT
          </span>
        </template>
      </el-table-column>
    </el-table>
    ...
    methods: {
      ...
      currentPageChange(e) {
        ...
        if(this.is_click==true){
          let data ={"chainId":this.value.toString(),"page":this.currentPage,"txtype":this.value1.toString(),"pagesize":10}
          ...
          let data ={"chainId":this.all_data.chainId,"page":this.currentPage,"hash":this.all_data.hash,"pagesize":10}
          getBlockNewTxPage(data).then(response => {
            if(response.data.transactionsList.dataList.length==0){
              this.tableData = []
              this.totla = 0
