    ...
    <div class="cc_all">
      <div class="cc_all_con">
        ...
        <div class="cc_all_con_right ">
          <span class="to_tr color_choose show_color_choose" v-show="is_zh" @click="to_lian(get_committeedata.chainId)">{{chainid_change_zh(get_committeedata.chainId)}}</span>
          <span class="to_tr color_choose show_color_choose" v-show="!is_zh"
                @click="to_lian(get_committeedata.chainId)">{{chainid_change_en(get_committeedata.chainId)}}</span>
        </div>
      </div>
      <div class="cc_all_con">
        <div class="cc_all_con_left">
        ...
        <div class="cc_all_con_right" style="user-select: none">
          ...
          <span class="el-icon-caret-left mcd_icon" @click="change_ehp(0)"></span>
          <span class=" el-icon-caret-right mcd_icon" @click="change_ehp(1)"></span>
        </div>
      </div>
      <div class="cc_all_con">
        <div class="cc_all_con_left">
      ...
      <div class="cc_all_con">
        ...
        <div class="cc_all_con_right cc_all_con_right_last">
                 <span class="show_color_choose" v-for=" item in get_committeedata.memberdetails" @click="cli_to_address(item)">
            {{item}}；&nbsp&nbsp&nbsp
          </span>
        </div>
      </div>
    ...
    >
      <el-table-column
        ...
        <template slot-scope="scope">
          <span class="to_tr show_color_choose" @click="to_bl_height_1(scope.row.chainId,scope.row.height,scope.row.hash)">{{scope.row.height}}</span>
        </template>
      </el-table-column>
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <span class="to_tr show_color_choose" @click="to_bl_height_1(scope.row.chainId,scope.row.height,scope.row.hash)">{{slice_hash(scope.row.hash)}}</span>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*点击单个参选成员=》地址详情跳转*/
      cli_to_address(e){
        // console.log(this.$store.getters.crosschain_to_block_detil.chainId)
