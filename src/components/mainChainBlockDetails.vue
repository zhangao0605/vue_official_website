<template>
  <div class="mcd_con">
    <div class="con_title">
      {{$t('table.own_chain')}}：
      <span v-show="is_zh">{{chainid_change_zh(is_ids)}}</span>
      <span v-show="!is_zh">{{chainid_change_en(is_ids)}}</span>
    </div>
    ...
    <div class="mcd_all">
      ...
      <div class="mcd_all_con">
        ...
        <div class="mcd_all_con_right color_choose">
          <span v-show="is_zh" @click="to_yifenpian(all_data.chainId)">{{chainid_change_zh(all_data.chainId)}}</span>
          <span v-show="!is_zh" @click="to_yifenpian(all_data.chainId)">{{chainid_change_en(all_data.chainId)}}</span>
        </div>
      </div>
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">
      ...
      <div class="mcd_all_con">
        ...
        <div class="mcd_all_con_right color_choose" @click="change_block(0)" style="  user-select: none;">
      ...
      <div class="mcd_all_con">
        ...
        <div class="mcd_all_con_right mcd_all_con_right_last color_choose">
          <span v-for=" item in memberdetails.memberdetails" @click="cli_to_address(item)">
            {{item}}；&nbsp&nbsp&nbsp
          </span>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*进入主链或者已分片子链界面*/
      to_weifenpian() {
        let data = {"chainId": this.$store.getters.crosschain_to_block_detil.chainId}
    ...
    created() {
      ...
      this.is_ids = this.$store.getters.crosschain_to_block_detil.chainId
